...
label = "blog"
title = "Blog"
url_namespace = app_name = "blog_app"
...
views = [
    ViewConfig(pattern="feed/<section>/<feed_type>/", template="app_list/blog/blog_list.html", name="blog_feed", pattern_kwargs={}, menu=False),
    ViewConfig(pattern="blogs/list/", template="app_list/blog/blog_list.html", name="blog", pattern_kwargs={}, current_section="all", post_list=post_list, section="all", feed_type="atom", is_paginated=True, page_obj=page_obj, paginator=paginator),
    #ViewConfig(pattern="blogs/list-empty/", template="app_list/blog/blog_list.html", name="blog_empty", pattern_kwargs={}, current_section="all", post_list=None),
    ViewConfig(pattern="blogs/section/<section>/", template="app_list/blog/blog_list.html", name="blog_section", pattern_kwargs={"section": "all"}, post_list=post_list, menu=False),
    ViewConfig(pattern="blogs/post/<int:post_pk>/", template="app_list/blog/blog_post.html", name="blog_post_pk", pattern_kwargs={"post_pk": 1}, post=post)
]
urlpatterns = [
    view.url()
    for view in views
]
url = path(r"", include("SaleSystemPortal.configs.blog", namespace=url_namespace))
