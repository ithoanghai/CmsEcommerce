...
class BlogIndexView(ListView):
    model = Post
    template_name = "app_list/blog/blog_list.html"
    search_parameter = "q"
...
class SlugUniquePostDetailView(DetailView):
    model = Post
    template_name = "app_list/blog/blog_post.html"
    slug_url_kwarg = "post_slug"

    def get(self, request, *args, **kwargs):
        if not settings.BLOG_SLUG_UNIQUE:
            raise Http404()
...
def blog_feed(request, **kwargs):
    ...
    scoper_lookup = kwargs.get(settings.BLOG_SCOPING_URL_VAR, None)
    ...
    if scoper_lookup is not None:
        blog_url_kwargs = {settings.BLOG_SCOPING_URL_VAR: scoper_lookup}

    if section and section != "all":
        section = get_object_or_404(Section, slug=section)
        feed_title = settings.BLOG_SECTION_FEED_TITLE % section.name
        posts = posts.filter(section=section)
    else:
        feed_title = settings.BLOG_FEED_TITLE
        section = settings.BLOG_ALL_SECTION_NAME

    if feed_type == "atom":
        feed_template = "app_list/blog/atom_feed.xml"
        feed_mimetype = "application/atom+xml"
    elif feed_type == "rss":
        feed_template = "app_list/blog/rss_feed.xml"
    ...
    blog_url = "http://{}{}".format(current_site.domain, reverse("blog:blog", kwargs=blog_url_kwargs))
    feed_url = "http://{}{}".format(current_site.domain, reverse("blog:blog_feed", kwargs=feed_url_kwargs))
...
class ManageSuccessUrlMixin:
    ...
    def get_success_url(self):
        scoping_lookup = self.kwargs.get(settings.BLOG_SCOPING_URL_VAR, None)
        if scoping_lookup:
            return reverse("blog:manage_post_list", kwargs={settings.BLOG_SCOPING_URL_VAR: scoping_lookup})
        return reverse("blog:manage_post_list")


class ManagePostList(ManageBlogMixin, ListView):

    model = Post
    template_name = "app_list/blog/manage_post_list.html"
...
class ManageCreatePost(ManageBlogMixin, ManageSuccessUrlMixin, CreateView):
    ...
    model = Post
    form_class = PostForm
    template_name = "app_list/blog/manage_post_create.html"
...
class ManageUpdatePost(ManageBlogMixin, ManageSuccessUrlMixin, UpdateView):
    ...
    form_class = PostForm
    pk_url_kwarg = "post_pk"
    template_name = "app_list/blog/manage_post_update.html"
...
class ManageDeletePost(ManageBlogMixin, ManageSuccessUrlMixin, DeleteView):
    ...
    model = Post
    pk_url_kwarg = "post_pk"
    template_name = "app_list/blog/manage_post_delete_confirm.html"
...
def ajax_preview(request, **kwargs):
    ...
    """
    data = {
        "html": render_to_string("app_list/blog/_preview.html", {
