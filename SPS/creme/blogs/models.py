...
STATES = settings.BLOG_UNPUBLISHED_STATES + ["Published"]
BLOG_STATE_CHOICES = list(zip(range(1, 1 + len(STATES)), STATES))


class Blog(models.Model):

    if settings.BLOG_SCOPING_MODEL is not None:
        scoper = models.ForeignKey(settings.AUTH_USER_MODEL, related_name="blogs",on_delete=models.CASCADE)
    ...
    def scoping_url_kwargs(self):
        if getattr(self, "scoper", None) is not None:
            return {settings.BLOG_SCOPING_URL_VAR: hookset.get_url_var(self.scoper)}
        return {}
...
class Post(models.Model):
    ...
    STATE_CHOICES = BLOG_STATE_CHOICES

    blog = models.ForeignKey(Blog, null= True, on_delete=models.CASCADE)
    section = models.ForeignKey(Section, on_delete=models.CASCADE)

    title = models.CharField(_("Title"), max_length=90)
    slug = models.SlugField(_("Slug"), max_length=90, unique=settings.BLOG_SLUG_UNIQUE)
    ...
    markup = models.CharField(_("Markup"), max_length=25, choices=settings.BLOG_MARKUP_CHOICES)

    teaser_html = models.TextField(editable=False)
    content_html = models.TextField(editable=False)

    description = models.TextField(_("Description"), blank=True)
    image_set = models.OneToOneField(ImageSet, null=True, related_name="blog_post", on_delete=models.CASCADE)
    ...
    def is_published(self):
        return self.state == BLOG_STATE_CHOICES[-1][0]
    ...
    def sharable_url(self):
        ...
        if not self.is_published or self.is_future_published:
            if self.secret_key:
                ...
                return reverse("blogs:blog_post_secret", kwargs=kwargs)
            else:
    ...
    def get_absolute_url(self):
        if self.is_published:
            if settings.BLOG_SLUG_UNIQUE:
                name = "blogs:blog_post_slug"
                kwargs = {
                    "post_slug": self.slug
                }
            else:
                name = "blogs:blog_post"
