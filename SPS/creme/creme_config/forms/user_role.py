...
from ...creme_core.forms import widgets as core_widgets
from ...creme_core.apps import (
    CremeAppConfig,
    creme_app_configs,
    extended_app_configs,
)
from ...creme_core.auth.entity_credentials import EntityCredentials
from ...creme_core.core import deletion
    ...
    def clean(self, *args, **kwargs):
        ...
        if not self._errors:
            ...
            if not any(get(fname) for fname in self.PERM_FIELDS.keys()):
                raise ValidationError(_('No action has been selected.'))
...
class UserRoleCloningForm(CremeModelForm):
    ...
    def __init__(self, role_to_clone: UserRole, *args, **kwargs):
        ...
        fields = self.fields
        fields['name'].initial = _('Copy of «{role}»').format(role=role_to_clone)
    ...
    def __init__(self, role_to_delete, *args, **kwargs):
        ...
        if CremeUser.objects.filter(role=role_to_delete).exists():
            ...
            self.fields['info'] = forms.CharField(
                label=_('Information'), required=False,
