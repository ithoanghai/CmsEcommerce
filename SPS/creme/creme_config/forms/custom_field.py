...
from ...creme_core.forms import fields as core_fields
from ...creme_core.core.deletion import FixedValueReplacer
from ...creme_core.creme_jobs import deletor_type
from ...creme_core.forms import CremeModelForm
from ...creme_core.forms.widgets import DynamicSelect
from ...creme_core.models import DeletionCommand, Job
...
class CustomEnumAddingForm(BaseCustomEnumAddingForm):
    choice = forms.CharField(label=_('New choice'))
...
class CustomEnumsAddingForm(BaseCustomEnumAddingForm):
    choices = forms.CharField(
        widget=Textarea(),
        label=_('New choices of the list'),
        help_text=_('Give the new possible choices (one per line).'),
    )
    ...
    def __init__(self, choice_to_delete, *args, **kwargs):
        ...
        if choice_to_delete.custom_field.value_class.objects.filter(
            ...
            self.fields['info'] = core_fields.ReadonlyMessageField(
                label=_('Information'),
                initial=_('This choice is not used by any entity, you can delete it safely.'),
            )
