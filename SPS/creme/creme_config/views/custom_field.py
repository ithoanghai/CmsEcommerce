...
from ...creme_core.views.bricks import (
    BricksReloading,
    BrickStateExtraDataSetting,
)
from ...creme_core.views.generic.base import EntityCTypeRelatedMixin
...
class CustomFieldCreation(EntityCTypeRelatedMixin,
    ...
    def get_title(self):
        return _('New custom field for «{model}»').format(
...
class CustomFieldEdition(base.ConfigModelEdition):
    ...
    def check_instance_permissions(self, instance, user):
        if instance.is_deleted:
            raise ConflictError(_('This custom field is deleted.'))
    ...
    def check_instance_permissions(self, instance, user):
        ...
        if dcom is not None:
            if dcom.job.status == Job.STATUS_OK:
                ...
                raise ConflictError(_('A deletion process for a choice already exists.'))
