...
class MailingListsTestCase(BrickTestCaseMixin, _EmailsTestCase):
    ...
    def _build_addcontact_url(mlist):
        return reverse('emails:add_contacts_to_mlist', args=(mlist.id,))

    @staticmethod
    def _build_addcontactfilter_url(mlist):
        return reverse('emails:add_contacts_to_mlist_from_filter', args=(mlist.id,))

    @staticmethod
    def _build_addorga_url(mlist):
        return reverse('emails:add_orgas_to_mlist', args=(mlist.id,))

    @staticmethod
    def _build_addorgafilter_url(mlist):
        return reverse('emails:add_orgas_to_mlist_from_filter', args=(mlist.id,))

    @staticmethod
    def _build_remove_from_campaign(campaign):
        return reverse('emails:remove_mlist_from_campaign', args=(campaign.id,))

    def test_create(self):
        user = self.login_as_root_and_get()

        url = reverse('emails:create_mlist')
    ...
    def test_ml_and_campaign01(self):
        ...
        url = reverse('emails:add_mlists_to_campaign', args=(campaign.id,))
    ...
    def test_recipients(self):
        ...
        url = reverse('emails:add_recipients', args=(mlist.id,))
    ...
    def test_add_recipients_from_csv(self, end):
        ...
        mlist = MailingList.objects.create(user=user, name='ml01')
        url = reverse('emails:add_recipients_from_csv', args=(mlist.id,))
        self.assertGET200(url)
    ...
    def test_recipients_error(self):
        ...
        self.assertGET404(reverse('emails:add_recipients', args=(orga.id,)))
    ...
    def test_ml_contacts01(self):
        ...
        self.client.post(
            reverse('emails:remove_contact_from_mlist', args=(mlist.id,)),
    ...
    def test_ml_orgas01(self):
        ...
        self.client.post(
            reverse('emails:remove_orga_from_mlist', args=(mlist.id,)),
    ...
    def test_ml_tree01(self):
        ...
        url = reverse('emails:add_child_mlists', args=(mlist01.id,))
        response1 = self.assertGET200(url)
        self.assertTemplateUsed(response1, 'creme_core/generics/blockform/link-popup.html')
        ...
        # --------------------
        self.assertPOST200(
            reverse('emails:remove_child_mlist', args=(mlist01.id,)),
    ...
    def test_ml_tree02(self):
        ...
        def post(parent, child):
            response = self.client.post(
                reverse('emails:add_child_mlists', args=(parent.id,)),
                data={'child': child.id},
            )
    ...
    def test_ml_tree03(self):
        ...
        user = self.login_as_root_and_get()
        orga = FakeOrganisation.objects.create(user=user, name='Dojo')
        self.assertGET404(reverse('emails:add_child_mlists', args=(orga.id,)))
