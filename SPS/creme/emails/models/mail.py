...
from ...creme_core.models.fields import UnsafeHTMLField
...
class AbstractMail(CremeModel):
    ...
    sending_date   = models.DateTimeField(_('Sending date'), null=True, editable=False, auto_now=True)
...
if not is_model_registered('reviews', 'Email'):
    class Email(AbstractMail):
        pass
...
class AbstractEntityEmail(AbstractMail, CremeEntity):
    ...
    def __str__(self):
        return _('Email <from: {sender}> <to: {to}> <status: {status}>').format(
            sender=self.sender,
            to=self.recipient,
