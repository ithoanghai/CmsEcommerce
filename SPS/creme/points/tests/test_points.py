...
class BasePointsTestCase(object):
    ...
    def tearDown(self):
        if hasattr(settings, "POINT_VALUES"):
            del settings.POINT_VALUES
...
class TopObjectsTagTestCase(BasePointsTestCase, TestCase):
    ...
    def test_no_args(self):
        try:
            Template("{% load points_tags %}{% top_objects %}")
    ...
    def test_typo_as(self):
        ...
        """
        try:
            Template('{% load points_tags %}{% top_objects "accounts.User" a top_users %}')
        except TemplateSyntaxError as e:
    ...
    #     t = Template('{% load points_tags %}{% top_objects "auth" as top_users %}')
    #     try:
    #         t.render(Context({}))
    #     except ValueError as e:
    #         self.assertEqual(str(e), "need more than 1 value to unpack")

    #     t = Template('{% load points_tags %}{% top_objects "auth.U" as top_users %}')
    #     try:
    #         t.render(Context({}))
    #     except ValueError as e:
    ...
    def test_should_return_annotated_queryset(self):
        t = Template("""{% load points_tags %}{% top_objects "accounts.User" as top_users %}""")
        c = Context({})
        t.render(c)
        self.assertEquals(c["top_users"].model, User.objects.all().model)

    def test_should_return_annotated_queryset_with_limit(self):
        t = Template("""{% load points_tags %}{% top_objects "accounts.User" as top_users limit 3 %}""")
        c = Context({})
        t.render(c)
        self.assertEquals(c["top_users"].model, User.objects.all().model)

    def test_should_return_annotated_queryset_non_user_model(self):
        t = Template("""{% load points_tags %}{% top_objects "auth.Group" as top_users %}""")
        c = Context({})
        try:
            t.render(c)
    ...
    def test_should_return_annotated_queryset_with_timeframe_has_points(self):
        t = Template("""{% load points_tags %}{% top_objects "accounts.User" as top_users timeframe 7 days %}""")  # noqa
        c = Context({})
        t.render(c)
    ...
    def test_should_return_annotated_queryset_with_timeframe_non_user_model_has_points(self):
        t = Template("""{% load points_tags %}{% top_objects "auth.Group" as top_users limit 10 timeframe 7 days %}""")  # noqa
        c = Context({})
        try:
            t.render(c)
            self.fail("Should have raised a NotImplementedError!")
...
class PointsForObjectTagTestCase(BasePointsTestCase, TestCase):
    ...
    def test_no_args(self):
        try:
            Template("{% load points_tags %}{% points_for_object %}")
        except TemplateSyntaxError as e:
    ...
    def test_type_as(self):
        try:
            self.setup_users(1)
            t = Template("{% load points_tags %}{% points_for_object user a points %}")
    ...
    def test_user_object_without_as(self):
        ...
        t = Template("{% load points_tags %}{% points_for_object user %} Points")
    ...
    def test_user_object_with_as(self):
        ...
        t = Template("{% load points_tags %}{% points_for_object user as points %}{{ points }} Points")  # noqa
    ...
    def test_user_object_with_limit(self):
        ...
        t = Template("{% load points_tags %}{% points_for_object user limit 7 days as points %}{{ points }} Points")  # noqa
    ...
    def test_user_object_with_limit_30_days(self):
        ...
        ap.save()
        award_points(self.users[0], 18)
        t = Template("{% load points_tags %}{% points_for_object user limit 30 days as points %}{{ points }} Points")  # noqa
