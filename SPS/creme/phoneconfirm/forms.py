...
class SelectDropdownWidget(forms.Select):
    ...
    def render_option(self, selected_choices, option_value, option_label):
        ...
        option_label_trans = get_template("app_list/phoneconfirm/_country_label.html").render(
            Context({"option_label": option_label})
        )

        return u'<option value="%s"%s>%s</option>' % (
            escape(option_value), selected_html,
            conditional_escape(smart_str(option_label_trans)))
...
class PhoneConfirmationForm(forms.Form):
    confirm_code = forms.CharField(
        ...
        max_length=100  # Thay đổi giá trị max_length tùy theo yêu cầu của bạn
    )
...
class PhoneNumberForm(forms.Form):
    country_code = forms.ChoiceField(
        ...
        required=False,
        widget=forms.Select,  # Sử dụng forms.Select thay vì SelectDropdownWidget
    )
