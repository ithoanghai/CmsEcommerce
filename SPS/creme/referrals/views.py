...
def create_referral(request):
    ...
    return JsonResponse({
        ...
        "html": render_to_string(
            "app_list/referrals/_create_referral_form.html",
            context=ctx,
            request=request
        )
    })
...
def process_referral(request, code):
    ...
    max_age = getattr(settings, "COOKIE_MAX_AGE", None)
    try:
        next_url = request.GET[
            getattr(settings, "REFERRALS_REDIRECT_ATTRIBUTE", "redirect_to")
        ]
    ...
    if request.user.is_anonymous:
        ...
        response.delete_cookie("referral")
