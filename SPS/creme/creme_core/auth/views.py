...
from django.views.generic import CreateView, FormView, RedirectView, TemplateView, View
...
class SignupView(PasswordMixin, FormView):
    ...
    def create_user(self, form, commit=True, model=None, **kwargs):
        CremeUser = model
        if CremeUser is None:
            CremeUser = get_user_model()
        user = CremeUser(**kwargs)
...
class RegisterView(CreateView):
    ...
    def post(self, request, *args, **kwargs):
        if CremeUser.objects.filter(email=request.POST['email']).exists():
