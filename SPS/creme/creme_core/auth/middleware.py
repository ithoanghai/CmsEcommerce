...
class ExpiredPasswordMiddleware(BaseMiddleware):
    ...
    def process_request(self, request):
        if request.user.is_authenticated and not request.user.is_staff:
            ...
            if next_url not in [settings.ACCOUNTS_LOGIN_REDIRECT_URL,
                                settings.ACCOUNTS_LOGOUT_REDIRECT_URL,
                                ]:
                if check_password_expired(request.user):
                    ...
                    change_password_url = reverse(settings.ACCOUNTS_LOGIN_REDIRECT_URL)
