...
from ...models.auth import EmailConfirmation, SignupCode
...
class SignupViewTestCase(TestCase):
    ...
    def test_closed(self):
        with self.settings(ACCOUNT_OPEN_SIGNUP=False):
            ...
            response = self.client.post(reverse("account_signup"), data)
            self.assertEqual(response.status_code, 200)
            self.assertEqual(response.template_name, "app_list/accounts/signup_closed.html")
    ...
    def test_get_authenticated(self):
        CremeUser.objects.create_user("foo", password="bar")
        self.client.login(username="foo", password="bar")

        with self.settings(ACCOUNTS_LOGIN_REDIRECT_URL="/logged-in/"):
            response = self.client.get(reverse("account_signup"))
...
class LoginViewTestCase(TestCase):
    ...
    def test_get(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.template_name, ["app_list/accounts/login.html"])
    ...
    @override_settings(
        AUTHENTICATION_BACKENDS=[
            "accounts.auth_backends.UsernameAuthenticationBackend",
        ]
    )
    def test_post_success(self):
        ...
        self.assertRedirects(
            response,
            settings.ACCOUNTS_LOGIN_REDIRECT_URL,
...
class LogoutViewTestCase(TestCase):
    ...
    def test_get_authenticated(self):
        ...
        response = self.client.get(reverse("account_logout"))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.template_name, ["app_list/accounts/logout.html"])
...
class ConfirmEmailViewTestCase(TestCase):
    ...
    def test_get_good_key(self):
        ...
        response = self.client.get(reverse("confirm_email", kwargs={"key": email_confirmation.key}))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.template_name, ["app_list/accounts/email_confirm.html"])

    def test_get_bad_key(self):
        response = self.client.get(reverse("confirm_email", kwargs={"key": "badkey"}))
        self.assertEqual(response.status_code, 404)

    @override_settings(ACCOUNT_EMAIL_CONFIRMATION_REQUIRED=True)
    def test_post_required(self):
        email_confirmation = self.signup()
        response = self.client.post(reverse("confirm_email", kwargs={"key": email_confirmation.key}), {})
        self.assertRedirects(
            response,
...
class ChangePasswordViewTestCase(TestCase):
    ...
    def signup(self):
        ...
        self.client.post(reverse("account_signup"), data)
        mail.outbox = []
        return CremeUser.objects.get(username="foo")
    ...
    def test_get_authenticated(self):
        ...
        response = self.client.get(reverse("account_password"))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.template_name, ["app_list/accounts/password_change.html"])
    ...
    def test_post_authenticated_success(self):
        ...
        self.assertRedirects(
            response,
            reverse(settings.PASSWORD_CHANGE_REDIRECT_URL),
            fetch_redirect_response=False
        )
    ...
    @override_settings(NOTIFY_ON_PASSWORD_CHANGE=False)
