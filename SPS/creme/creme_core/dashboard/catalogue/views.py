...
from ...views.generic.base import ObjectLookupView
...
(StockRecordFormSet,
 ...
 AttributeOptionFormSet) \
    = get_classes('creme_core.dashboard.catalogue.formsets',
                  ('StockRecordFormSet',
                   'ProductCategoryFormSet',
                   'ProductImageFormSet',
                   'ProductRecommendationFormSet',
...
ProductTable, CategoryTable, AttributeOptionGroupTable, OptionTable \
    = get_classes('creme_core.dashboard.catalogue.tables',
 ...
 PopUpWindowDeleteMixin) \
    = get_classes('creme_core.views.index',
                  ('PopUpWindowCreateMixin',
                   'PopUpWindowUpdateMixin',
                   'PopUpWindowDeleteMixin'))
PartnerProductFilterMixin = get_class('creme_core.dashboard.catalogue.mixins', 'PartnerProductFilterMixin')
...
class ProductCreateRedirectView(generic.RedirectView):
    ...
    def get_product_create_url(self, product_class):
        ...
        return reverse('catalogue-product-create',
    ...
    def get_invalid_product_class_url(self):
        ...
        return reverse('catalogue-product-list')
...
class ProductCreateUpdateView(PartnerProductFilterMixin, generic.UpdateView):
    ...
    def check_objects_or_redirect(self):
        ...
        if self.creating and self.parent is not None:
            ...
            if not is_valid:
                ...
                return redirect('catalogue-product-list')
    ...
    def get_success_url(self):
        ...
        if action == 'continue':
            url = reverse(
                'catalogue-product', kwargs={"pk": self.object.id})
        elif action == 'create-another-child' and self.parent:
            url = reverse(
                'catalogue-product-create-child',
            ...
            url = reverse('catalogue-product-list')
...
class CategoryUpdateView(CategoryListMixin, generic.UpdateView):
    ...
    def get_success_url(self):
        ...
        action = self.request.POST.get('action')
        if action == 'continue':
            return reverse('catalogue-category-update', kwargs={"pk": self.object.id})
        return super().get_success_url()
