...
from . import CremeEntity, fields as core_fields

if TYPE_CHECKING:
    from ..core.entity_filter.condition_handler import (
...
class EntityFilter(models.Model):  # TODO: CremeModel? MinionModel?
    ...
    def can_delete(self, user) -> tuple[bool, str]:
        if not self.is_custom:
            return False, _("This filter can't be edited/deleted")
    ...
    def can_edit(self, user) -> tuple[bool, str]:
        ...
        if self.filter_type != EF_USER:
            return False, _('You cannot edit/delete a system filter')
        ...
        if not user.has_perm(self.entity_type.app_label):
            return False, _('You are not allowed to access to this app')
        ...
        return False, _('You are not allowed to view/edit/delete this filter')
    ...
    def check_cycle(self, conditions: Iterable[EntityFilterCondition]) -> None:
        ...
        if self.id in ref_filter_ids:
            raise EntityFilter.CycleError(
                _('A condition can not reference its own filter.')
            )

        # TODO: method intersection not null
        if self.get_connected_filter_ids() & ref_filter_ids:
            raise EntityFilter.CycleError(
                _('There is a cycle with a sub-filter.')
            )
