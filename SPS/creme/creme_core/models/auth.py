...
from ..auth.signals import signup_code_sent, signup_code_used
from ..auth.fields import TimeZoneField
from ..auth.entity_credentials import EntityCredentials
from ..auth.languages import DEFAULT_LANGUAGE
from ..auth.managers import EmailAddressManager, EmailConfirmationManager
from ..auth.hooks import hookset
...
class SetCredentials(models.Model):
    ...
    role = models.ForeignKey(
        UserRole,
        related_name='credentials', verbose_name=_('Role'), null=True, on_delete=models.CASCADE, editable=False,
    ...
    def __str__(self):
        ...
        if not perms:
            append(
                _('nothing forbidden') if forbidden else
                _('nothing allowed')
            )
        ...
        if self.ctype:
            ...
            format_str = (
                _('For “{set}“ of type “{type}” it is forbidden to: {perms}')
                if forbidden else
                _('For “{set}“ of type “{type}” it is allowed to: {perms}')
            )
        else:
            format_str = (
                _('For “{set}“ it is forbidden to: {perms}')
                if forbidden else
                _('For “{set}“ it is allowed to: {perms}')
            )
...
class CremeUser(AbstractUser, PermissionsMixin):
    ...
    email = models.EmailField(_('Email address'), max_length=150, unique=True, blank=False, error_messages={'unique': "A user with that email already exists.",})
    ...
    def get_full_name(self) -> str:
        if self.is_team:
            return _('{user} (team)').format(user=self.username)
    ...
    def _get_app_verbose_name(app_label: str) -> str:
        try:
            ...
            return _('Invalid app "{}"').format(app_label)
    ...
    def has_perm_to_delete_or_die(self, entity: CremeEntity) -> None:
        if not self.has_perm_to_delete(entity):
            raise PermissionDenied(
                _('You are not allowed to delete this entity: {}').format(
                    entity.allowed_str(self),
                )
            )
    ...
    def has_perm_to_export_or_die(self,
        ...
        if not self.has_perm_to_export(model_or_entity):
            raise PermissionDenied(
                _('You are not allowed to export: {}').format(
                    model_or_entity._meta.verbose_name
                )
            )
    ...
    def has_perm_to_link_or_die(self,
        ...
        if not self.has_perm_to_link(entity_or_model, owner):
            if isinstance(entity_or_model, CremeEntity):
                msg = _('You are not allowed to link this entity: {}').format(
                    entity_or_model.allowed_str(self)
                )
            else:
                msg = _('You are not allowed to link: {}').format(
    ...
    def has_perm_to_unlink_or_die(self, entity: CremeEntity) -> None:
        if not self.has_perm_to_unlink(entity):
            raise PermissionDenied(
                _('You are not allowed to unlink this entity: {}').format(
                    entity.allowed_str(self),
                )
            )
...
class Account(models.Model):
    ...
    language = models.CharField(
        _("language"),
        max_length=10,
        choices=settings.AUTH_LANGUAGES,
...
class SignupCodeResult(models.Model):
    ...
    class Meta:
        verbose_name = _("signup code result")
        verbose_name_plural = _("signup codes result")
        app_label = 'creme_core'
