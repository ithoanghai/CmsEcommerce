...
from ..creme_core.core.utils import get_default_currency
from ..creme_core.models.fields import AutoSlugField

from . import exceptions

logger = logging.getLogger('creme.order')
...
class AbstractOrder(models.Model):
    ...
    def verification_hash(self):
        signer = Signer(salt='creme.order.Order')
...
class AbstractOrderStatusChange(models.Model):
    ...
    date_created = models.DateTimeField(_('Date Created'), auto_now=True, db_index=True)
...
class AbstractCommunicationEvent(models.Model):
    ...
    date_created = models.DateTimeField(_("Date"), auto_now=True, db_index=True)
...
class AbstractPaymentEvent(models.Model):
    ...
    * 2 lines have been refunded
    ...
    event_type = models.ForeignKey(
        'order.PaymentEventType', null=True, blank=True,
        on_delete=models.CASCADE,
        verbose_name=_("Event Type"))
...
class AbstractShippingEvent(models.Model):
    ...
    event_type = models.ForeignKey(
        'order.ShippingEventType', null=True, blank=True,
        on_delete=models.CASCADE,
        verbose_name=_("Event Type"))
