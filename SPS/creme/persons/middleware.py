...
from ..creme_core.auth.utils import handle_redirect_to_login
...
class TeamMiddleware:
    ...
    def process_request(self, request):
        team_slug = request.environ.get("team")
        if team_slug is not None:
            ...
            request.team = None
        if request.user.is_authenticated and settings.PERSONS_PROFILE_MODEL:
            if re.search(r"^/teams/[\w-]+/account/signup/", request.path):
                return None
            profiles = settings.PERSONS_PROFILE_MODEL.objects.filter(
                user=request.user
            )
            ...
            except settings.PERSONS_PROFILE_MODEL.DoesNotExist:
                raise Http404()
        else:
            if team_slug is not None:
                return check_team_allowed(request)
