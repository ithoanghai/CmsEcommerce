...
Chạy các lệnh sau (cài đặt mới VÀ nâng cấp từ phiên bản trước):
 ...
 To build the static assets (JavaScript, CSS, images...):
   > creme generatemedia --settings=my_project.settings
 You COULD now run a DEVELOPMENT Web server to test the previous steps before  configuring a production a Web server:
   > creme runserver --settings=my_project.settings
Do not forget that some features need the job manager to be launched; you should configure a watch-dog for the following command:
...
Then add a field 'status' in our model ``Beaver``: ::
    ...
    from ...creme.creme_core.models.entity import CremeEntity, CREME_REPLACE  # <- NEW
